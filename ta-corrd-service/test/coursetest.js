...
describe('course', function(){
  ...
  after(function(done){
    Course.remove({course_code: "CSC500H1S"}, (err)=>{
      if(err) throw err;
    });
    Course.remove({course_code: "CSC600H1S"}, (err)=>{
      if(err) throw err;
    })
    done();
  });
  ...
  it('should successfully create a new course', function(done){
    ...
    .end(function(err,res){
      ...
      Course.remove({_id: res.body.course._id}, (err)=>{
        if(err) throw err;
      });
      done();
    });
